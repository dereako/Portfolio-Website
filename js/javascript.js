...
var loading = false;
var done = true;
var projects = Array(1,4,13,24,11,15,23,19,20,21,3,18,17,2,14);
var currently = 0;
var speed = 300;
var timer = "";
var i = 1;
...
function loadProj() {
	if (loading==false) {
		loading = true;
		$.post('load.php',{'proj':projects[p],'width':$('.list').width()},function(data) {
...
function getCurrent() {
	...
	if ($(window).scrollTop() + $(window).height() >= $(document).height() - Math.floor(parseInt($('.project:last').css('padding-bottom')))) {
		...
		for(var i=1; i<=projects.length; i++) {
			if ((top >= topList[i-1] && topList[i-1]) && top < topList[i]) {
				return i;
			}
		}
	}
...
function goTo(which) {
	if (done==true && allLoaded==true) {
		...
		var newY = (topList[which] - Math.round(parseInt($('.list').css("padding-top")))) +"px";
...
$(document).keydown(function (e) {
	...
	switch (keyCode) {
		...
		break;
		case arrow.up:
			if (currently > 0) { goTo(currently-1); } else { goTo(projects.length-1); }
			return false;
		break;
		case arrow.right:
			if (speed > 20) { speed -= 20; }
		...
		break;
		case arrow.down:
			if (currently < projects.length-1) { goTo(currently+1); } else { goTo(0); }
			return false;
		break;
	}
});
